...
A live hand vector **v\_live** is projected onto each class's subspace:

$$v\_{proj} = UU^T(v\_{live} - \\mu)$$

The **Euclidean distance** (reconstruction error) is calculated:

$$d = |(v\_{live} - \\mu) - v\_{proj}|$$
...
1. Download the [American Sign Language Image Dataset](https://www.kaggle.com/datasets/shubhambhardwaj01/american-sign-language-image-dataset) from Kaggle.
2. Rename the root folder to `word\\\\\\\_dataset` and place it in the project directory.
3. Run the reorganization script to restructure it:
...
* Understand and be able to explain the full math pipeline:
  ...
  * Why we center the data: $X\_c = X - \\mu$
  * How the Covariance Matrix is computed: $C = X\_c^T X\_c / (n-1)$
